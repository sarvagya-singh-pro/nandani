...
import network
url="http://9cb0-35-193-188-65.ngrok-free.app/pico"
import socket
import urequests
...
from time import sleep
from pico_i2c_lcd import I2cLcd
i2c1 = I2C(0, sda=Pin(8), scl=Pin(9), freq=100000)
I2C_ADDR = i2c1.scan()[0]
lcd = I2cLcd(i2c1, I2C_ADDR, 2, 16)
lcd.show_cursor()
lcd.display_on()
    ...
    while True:
        ...
        if sensor.available():
            ...
            if response.status_code == 200:
                    ...
                    lcd.clear()
                    lcd.putstr("Hr:"+str(response.json()["ok"][0]))
                    sleep(1.5)
                    lcd.clear()
                    lcd.putstr("Ox:"+str(response.json()["ok"][1]))
                    sleep(1.5)
                    lcd.clear()
                    lcd.putstr("Temp:"+str(sensor.read_temperature()))
            else:
